...
public class MiniJSON
{
	...
	public static string jsonEncode(object json)
	{
		...
		return (success ? builder.ToString().Replace("\"{\\\"", "[{\"").Replace("\\\":", "\":").Replace("}\"", "}]") : null);
	}
...
{
	public static string toCustomDataJson(this Hashtable obj)
	{
		string finalString = "[";
		int i = 0;
		foreach (DictionaryEntry pair in obj)
		{
			Hashtable formattedData = new Hashtable();
			formattedData.Add("key", pair.Key);
			formattedData.Add("value", pair.Value);
			if (i == 0)
			{
				finalString += MiniJSON.jsonEncode(formattedData);
			}
			else
			{
				finalString += "," + MiniJSON.jsonEncode(formattedData);
	...
	{
		string formattedJson = json.Replace("\"[", "[").Replace("\\", "").Replace("]\"", "]");
		return MiniJSON.jsonDecode(formattedJson) as ArrayList;
	}

	public static Hashtable hashtableFromJson(this string json)
	{
		string formattedJson = json.Replace("\"[", "[").Replace("\\", "").Replace("]\"", "]");
		return MiniJSON.jsonDecode(formattedJson) as Hashtable;
	}
}
